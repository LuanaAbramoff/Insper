...
with open(outputBIN, "w+") as f:  #Abre o destino BIN
    ...
    for line in lines_sem_labels:        
        ...
        #Se a linha for válida para conversão, executa
            ...
            if '.' in instrucaoLine:  # Identifica referência a label
                parts = instrucaoLine.split()
                instruction, label = parts[0], parts[1].lstrip('.')  # Separa a instrução do label, removendo o '.' do início do label
                if label in labels_map:  # Substitui label pelo número de linha correspondente
                    instrucaoLine = f"{instruction} @{labels_map[label]}"
                else:
                    print(f"Label '{label}' não encontrado na linha: {line.strip()}")
                    continue  # Pula para a próxima linha se o label não for encontrado
